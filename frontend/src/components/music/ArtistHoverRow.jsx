...
const ArtistHoverRow = ({ artist, index }) => {
    const [imageError, setImageError] = useState(false);
    const [isHovered, setIsHovered] = useState(false);

    const photoUrl = artist.photoUrl || artist.avatar;
    const hasPhoto = photoUrl && !photoUrl.includes('placeholder') && !imageError;
    ...
    return (
        ...
        >
            <Link to={`/curator/${artist._id}`} className="absolute inset-0 z-20"></Link>
            ...
            >
                {hasPhoto ? (
                    <div className="w-full flex justify-center bg-black/10">
                        <img
                            src={photoUrl}
                            alt={artist.name}
                            className="w-full md:w-auto max-h-[85vh] object-contain grayscale hover:grayscale-0 transition-all duration-1000 ease-out"
                            onError={() => setImageError(true)}
                        />
                    </div>
                ) : (
                    <div className="w-full h-[600px] flex flex-col items-center justify-center bg-muted grayscale">
                        <User size={120} className="text-foreground/10" strokeWidth={0.5} />
                        <span className="serif-display text-4xl md:text-8xl lg:text-[12rem] text-foreground/5 absolute uppercase tracking-tighter select-none">
                            {artist.name}
                        </span>
                    </div>
                )}
                ...
                <div className="absolute bottom-12 left-12 z-20">
                    <p className="text-[10px] tracking-[0.5em] text-white/60 mb-2 font-bold uppercase drop-shadow-md">SONIC ARCHITECT</p>
                    <h3 className="serif-display text-4xl text-white italic drop-shadow-md">{artist.name}</h3>
                </div>
            </div>
        </motion.div>
    );
